<?php

define('TIME_OUT_CLEAR', 1 * 60 * 30);

define('DIR_CONTROLLER', THEME_URL . DS . 'controllers' . DS);
define('DIR_MODEL', THEME_URL . DS . 'models' . DS);
define('DIR_VIEW', THEME_URL . DS . 'views' . DS);
define('DIR_CLASS', THEME_URL . DS . 'class' . DS);
define('DIR_TAXONOMY', THEME_URL . DS . 'taxonomies' . DS);
define('DIR_META_BOX', THEME_URL . DS . 'metabox' . DS);
define('DIR_IMAGES', THEME_URL . DS . 'image' . DS);
define('DIR_ICON', DIR_IMAGES . 'icon' . DS);
define('DIR_FILE', THEME_URL . DS . 'file' . DS);
define('DIR_LANGUAGES', THEME_URL . DS . 'languages' . DS);
define('DIR_HELPER', THEME_URL . DS . 'helper' . DS);


// duong part
define('PART_IMAGES', THEME_PART . '/image/');
define('PART_ICON', PART_IMAGES . '/icon/');
define('PART_FILE', THEME_PART . '/file/');
define('PART_CLASS', THEME_PART . '/class/');
